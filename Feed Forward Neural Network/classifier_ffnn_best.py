    ...
    while loss_cur >= 0.45:
        for batch_n in range(batch_count):
            ...
            if i % (batch_count * 10) == 0:
                ...
                if not flag:
                    loss_prev = loss_cur
                loss_cur = loss(train_set, labels, W1, W2, W3, REG_PARAM)
                #print(loss_cur)
                if(loss_cur > loss_prev):
                    ...
                    W3_p = W3
                    flag = False
            g1, g2, g3 = backpropagation(X_cur, y_cur, W1, W2, W3, REG_PARAM)
            W1, W2, W3 = update_weights(W1, W2, W3, g1, g2, g3, LEARNING_RATE)
            i += 1
    return glove, W1, W2, W3



def classify(texts, params):
    glove, W1, W2, W3 = params
    ...
    for i, text in enumerate(vectorized_texts):
        _, _, _, _, pred = feedforward(test_set[i,:], W1, W2, W3)
        pred = np.sum(pred)
        if pred >= 0.5:
            res.append('pos')
        else:
